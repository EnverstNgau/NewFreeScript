...
local Window = Fluent:CreateWindow({
    Title = "Lùa Hub | Next Generation | V2.0 ",
...
spawn(function()
    while wait() do
        AntiBan()
        task.wait()
        posX = math.random(15,30)
        posY = math.random(15,20)
        posZ = math.random(15,30)
        wait()
        Pos = Cframe.new(posX, posY, posZ)
        Pos2 = Cframe.new(posX, posY, posZ)
    end
end)
...
function BTP(p)
    pcall(function()
        if (p.Position-game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude >= 3000 and not Auto_Raid and game.Players.LocalPlayer.Character.Humanoid.Health > 0 then
...
local ToggleCL = Tabs.Main:AddToggle("ToggleCL", {Title = "Auto Collect Azure", Default = false })
        ...
        while wait(.1) do
            pcall(function()
                if _G.BuyLengendSword or Triple_A then
                    bananadepzaihayvandodz = {1,3}
                    local args = {
                        [1] = "LegendarySwordDealer",
                        [2] = bananadepzaihayvandodz
                    }
                    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack(args))
                else
                    wait(2)
...
function getHits(Size)
    ...
    for i = 1, #Enemies do
        ...
        if Human and Human.RootPart and Human.Health > 0 and game.Players.LocalPlayer:DistanceFromCharacter(Human.RootPart.Position) < Size + 15 then
...
function getAllBladeHits(Sizes)
    ...
    for i=1,#Enemies do local v = Enemies[i]
        local Human = v:FindFirstChildOfClass("Humanoid")
        if Human and Human.RootPart and Human.Health > 0 and Client:DistanceFromCharacter(Human.RootPart.Position) < Sizes + 55  then
            table.insert(Hits,Human.RootPart)
        end
    end
    for i=1,#Characters do local v = Characters[i]
        local Human = v:FindFirstChildOfClass("Humanoid")
        if v.Name ~= game.Players.LocalPlayer.Name and Human and Human.RootPart and Human.Health > 0 and Client:DistanceFromCharacter(Human.RootPart.Position) < Sizes + 55 then
            table.insert(Hits,Human.RootPart)
        end
    end
    return Hits
...
function Attack()
    task.spawn(function()
		FF = getupvalues(require(game:GetService("Players").LocalPlayer.PlayerScripts.CombatFramework))[2]
        local ac = SeraphFrame.activeController
        if ac and ac.equipped then
            task.spawn(function()
                CamShake:Stop()
                Animation.AnimationId = ac.anims.basic[2]
                ac.humanoid:LoadAnimation(Animation):Play(0.1, 0.1, 0.1)
                game:GetService("ReplicatedStorage").RigControllerEvent:FireServer("hit",getHits(180),2,"")
				FF.activeController.hitboxMagnitude = 2480
				FF.activeController.timeToNextAttack = 0
				FF.activeController.timeToNextBlock = 0
				FF.activeController.increment = 3
				FF.activeController.attacking = false
				FF.activeController.blocking = false
				FF.activeController:attack()
			end)
		end
	end)
end
				...
				for i, v in pairs(game.Workspace.Enemies:GetChildren()) do
					if v.Humanoid.Health > 0 and v:FindFirstChild("Humanoid")and v:FindFirstChild("HumanoidRootPart") then
						repeat task.wait()
                            ...
                            CamShake:Stop()
						until not _G.FastAttack or not _G.SuperFastAttack
					end
				end
			end)
		end
...
CamShake:Stop()
local RadiusBring = Tabs.Setting:AddSlider("RadiusBring", {
    Title = "Radius Bring Mob / Magnet",
    Description = "",
    Default = 350,
    Min = 0,
    Max = 500,
    Rounding = 1,
    Callback = function(RadiusBring)
        RadiusBring = RadiusBring
    end
})

RadiusBring:OnChanged(function(RadiusBring)
    RadiusBring = RadiusBring
end)
                ...
                if b:FindFirstChild("Humanoid") and b:FindFirstChild("HumanoidRootPart") then
                    if isnetworkowner(b:FindFirstChild("HumanoidRootPart"))then
                        ...
                        if b:FindFirstChild("Humanoid"):FindFirstChild("Animator")then
                            ...
                            b.Humanoid:ChangeState(15)
                            b:FindFirstChild("HumanoidRootPart").CFrame = F
                        elseif (b.Position-d.HumanoidRootPart.Position).magnitude < 300 then
                            b:FindFirstChild("HumanoidRootPart").CanCollide = false
                            b:FindFirstChild("HumanoidRootPart").Transparency = 1
                            if not b:FindFirstChild("HumanoidRootPart"):FindFirstChild("BV")then
...
task.spawn(function()
    while task.wait() do
        if _G.BringMob then
            pcall(function()
                for i,v in pairs(game.Workspace.Enemies:GetChildren()) do
                    if not string.find(v.Name,"Boss") and v.Name == MonFarm and (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= RadiusBring then
...
local ToggleBypassTP = Tabs.Setting:AddToggle("ToggleBypassTP", {Title = "Enabled Fast Teleport", Default = false })
...
ToggleTeleport:OnChanged(function(Value)
    ...
    pcall(function()
        if _G.TeleportPly then
            repeat toTarget(game:GetService("Players")[_G.SelectPly].Character.HumanoidRootPart.CFrame*Pos) wait() NormalClick() until _G.TeleportPly == false
        end
    end)
end)
...
ToggleQuanSat:OnChanged(function(Value)
    ...
    game:GetService("Workspace").Camera.CameraSubject = plr1
end)
...
if First_Sea or Second_Sea then
    local Mastery = Tabs.Hop:AddSection("Go To Third Sea")
elseif Third_Sea then
    ...
    local Togglelockmoon = Tabs.Hop:AddToggle("Togglelockmoon", {Title = "Lock Moon and Use Race Skill", Default = false })
